/**
 * care-shop.js
 * Product rendering and filtering for CKP Care Shop
 */
...
(function() {
    ...
    async function init() {
        console.log('[CareShop] ✅ Script loaded');
    ...
    async function loadProducts() {
        try {
            ...
            console.error('[CareShop] Failed to load products:', err);
        ...
        return {
            divisions: {
                care: {
                    ...
                    categories: [
                        { id: 'wound-care',       name: { de: 'Wundversorgung',     en: 'Wound Care'       } },
                        { id: 'emergency',        name: { de: 'Notfallausrüstung',  en: 'Emergency'        } },
                        { id: 'cardiac',          name: { de: 'Herz-Kreislauf',     en: 'Cardiac'          } },
                        { id: 'diagnostics',      name: { de: 'Diagnostik',         en: 'Diagnostics'      } },
                        { id: 'protection',       name: { de: 'Schutzausrüstung',   en: 'Protection'       } },
                        { id: 'hygiene',          name: { de: 'Hygiene',            en: 'Hygiene'          } },
                        { id: 'bandages',         name: { de: 'Verbandsmaterial',   en: 'Bandages'         } },
                        { id: 'medical-supplies', name: { de: 'Medizinbedarf',      en: 'Medical Supplies' } },
    ...
    function renderProductCard(product) {
        ...
        const imgHTML = images.map((src, i) => `
            <img src="${src}" alt="${name}" class="product-image ${i === 0 ? 'active' : ''}" data-index="${i}"
                onerror="this.onerror=null; this.src='https://via.placeholder.com/400x400/e8f4fd/1a6fa8?text=${encodeURIComponent(name)}';">
        `).join('');
    ...
    function bindCardEvents() {
        document.querySelectorAll('.product-card').forEach(card => {
            ...
            // ── "ANFRAGE STELLEN" button only — not the whole card ───────────────
    ...
    function attachEventListeners() {
        ...
        document.getElementById('sort-select')?.addEventListener('change', function() {
            ...
            bindCardEvents(); // re-bind after sort reorder
        });
    }
