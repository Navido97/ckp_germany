...
(function() {
    ...
    function createPopup() {
        const popupHTML = `
                    ...
                    <div class="ww-grid">
                        <div class="ww-product-side">
                            ...
                            <div class="ww-image-area">
                                ...
                                <button class="ww-nav ww-nav-prev" style="display:none;"><svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"></polyline></svg></button>
                                <button class="ww-nav ww-nav-next" style="display:none;"><svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg></button>
                            ...
                            <div class="ww-product-info">
                                ...
                                <div id="ww-product-features" class="ww-features"><h4>PRODUKTSPEZIFIKATIONEN</h4><ul id="ww-features-list"></ul></div>
                            </div>
                        </div>
                        <div class="ww-form-side">
                            ...
                            <form id="ww-inquiry-form" class="ww-form" name="workwear-inquiry" netlify netlify-honeypot="bot-field">
                                <input type="hidden" name="form-name" value="workwear-inquiry">
                                <input type="hidden" name="bot-field" style="display:none">
                                <input type="hidden" name="product" id="ww-hidden-product">
                                <input type="hidden" name="sku" id="ww-hidden-sku">
                                <div class="ww-row-2">
                                    <div class="ww-group"><label id="lbl-name">Vollständiger Name</label><input type="text" name="name" required></div>
                                    <div class="ww-group"><label id="lbl-org">Unternehmen / Organisation</label><input type="text" name="organization" required></div>
                                </div>
                                <div class="ww-row-2">
                                    <div class="ww-group"><label id="lbl-email">E-Mail-Adresse</label><input type="email" name="email" required></div>
                                    <div class="ww-group"><label id="lbl-phone">Telefonnummer</label><input type="tel" name="phone"></div>
                                </div>
                                <div class="ww-group"><label id="lbl-quantity">Benötigte Menge</label><input type="number" name="quantity" value="1" min="1"></div>
                                <div class="ww-group"><label id="lbl-message">Besondere Anforderungen oder Nachricht</label><textarea name="message" rows="4"></textarea></div>
            ...
            <style>
                .ww-inquiry-popup { position:fixed; top:0; left:0; right:0; bottom:0; z-index:10001; display:none; overflow-y:auto; }
                .ww-inquiry-popup.active { display:block; }
                .ww-overlay { position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.9); backdrop-filter:blur(8px); animation:wwFadeIn 0.3s ease; }
                .ww-content { position:relative; max-width:1100px; margin:2rem auto; background:linear-gradient(135deg,#1e2a1e 0%,#243024 100%); border-radius:18px; border:1px solid rgba(107,142,35,0.3); overflow:hidden; animation:wwSlideUp 0.4s cubic-bezier(0.175,0.885,0.32,1.275); }
                .ww-close { position:absolute; top:1.5rem; right:1.5rem; width:40px; height:40px; background:rgba(0,0,0,0.4); border:none; border-radius:50%; color:white; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all 0.3s ease; z-index:10; }
                .ww-close:hover { background:#6b8e23; transform:rotate(90deg); }
                .ww-grid { display:grid; grid-template-columns:55% 45%; }
                .ww-product-side { background:rgba(0,0,0,0.35); padding:2.5rem; }
                .ww-back { display:inline-flex; align-items:center; gap:0.5rem; background:none; border:none; color:#6b8e23; font-size:0.82rem; font-weight:600; letter-spacing:1px; cursor:pointer; margin-bottom:1.75rem; transition:gap 0.2s ease; }
                .ww-back:hover { gap:0.75rem; }
                .ww-image-area { width:100%; height:480px; background:rgba(0,0,0,0.3); border-radius:12px; overflow:hidden; margin-bottom:2rem; position:relative; }
                .ww-images-container { position:relative; width:100%; height:100%; }
                .ww-img { position:absolute; top:0; left:0; width:100%; height:100%; object-fit:cover; opacity:0; transition:opacity 0.5s ease; }
                .ww-img.active { opacity:1; }
                .ww-nav { position:absolute; top:50%; transform:translateY(-50%); z-index:10; background:rgba(0,0,0,0.55); border:none; color:white; width:46px; height:46px; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; transition:all 0.3s ease; }
                .ww-nav:hover { background:#6b8e23; transform:translateY(-50%) scale(1.1); }
                .ww-nav-prev { left:14px; }
                .ww-nav-next { right:14px; }
                .ww-dots { position:absolute; bottom:16px; left:50%; transform:translateX(-50%); display:flex; gap:8px; z-index:10; }
                .ww-dot { width:9px; height:9px; border-radius:50%; background:rgba(255,255,255,0.4); cursor:pointer; transition:all 0.3s ease; }
                ...
                .ww-group input, .ww-group textarea { width:100%; background:rgba(0,0,0,0.35); border:1px solid rgba(107,142,35,0.3); color:#e8f0e0; padding:0.8rem; border-radius:8px; font-size:0.92rem; font-family:'Inter',sans-serif; transition:all 0.3s ease; box-sizing:border-box; }
                .ww-group input:focus, .ww-group textarea:focus { outline:none; border-color:#6b8e23; background:rgba(0,0,0,0.5); }
                .ww-group textarea { resize:vertical; }
                .ww-actions { display:flex; gap:1rem; margin-top:0.5rem; }
                .ww-btn-cancel { flex:1; padding:0.9rem; background:transparent; border:1px solid rgba(107,142,35,0.3); color:#9ab087; border-radius:8px; cursor:pointer; font-size:0.92rem; font-weight:600; transition:all 0.3s ease; }
                .ww-btn-cancel:hover { border-color:#6b8e23; color:#e8f0e0; }
                .ww-btn-submit { flex:2; padding:0.9rem 1.5rem; background:linear-gradient(135deg,#6b8e23 0%,#8aad3a 100%); border:none; color:white; border-radius:8px; cursor:pointer; font-size:0.92rem; font-weight:700; display:flex; align-items:center; justify-content:center; gap:0.6rem; transition:all 0.3s ease; }
                ...
                .ww-security { display:flex; gap:1.5rem; margin-top:1.25rem; padding-top:1.25rem; border-top:1px solid rgba(107,142,35,0.2); color:#5a7a4a; font-size:0.75rem; letter-spacing:0.5px; }
                .ww-success { display:flex; align-items:center; gap:1rem; padding:1.15rem; border-radius:8px; margin-top:1.5rem; background:rgba(107,142,35,0.15); border:1px solid rgba(107,142,35,0.4); color:#8aad3a; }
                .ww-success.hidden { display:none; }
                @keyframes wwFadeIn { from{opacity:0} to{opacity:1} }
                @keyframes wwSlideUp { from{opacity:0;transform:translateY(50px) scale(0.95)} to{opacity:1;transform:translateY(0) scale(1)} }
    ...
    function openPopup() {
        ...
        fillProduct();
        updateLabels();
        document.getElementById('ww-hidden-product').value = currentProduct.name[currentLanguage];
        document.getElementById('ww-hidden-sku').value     = currentProduct.sku;
    }
    ...
    function fillProduct() {
        ...
        if (images.length > 1) {
            prevBtn.style.display = nextBtn.style.display = dotsCont.style.display = 'flex';
            ...
            prevBtn.style.display = nextBtn.style.display = dotsCont.style.display = 'none';
        }
